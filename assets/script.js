...
for (let i = 0; i < 9; i++) {
  ...
  let input;
  let fI = getInput('input-'+i)

  
  if (i + 9 < m.format("H")) {
    input =
      `<input id='input-${i}' type='text' class='text-center past time-block'/>`;
  } else if (i + 9 === Math.floor(m.format("H"))) {
    input =
      `<input id='input-${i}' type='text' class='text-center present time-block'>`;
  } else {
    input =
      `<input id='input-${i}' type='text' class='text-center future time-block'>`;
  }
